#!/usr/bin/env python3
"""
GitHub Forks Data Fetcher using GraphQL

Fetches forks data from GitHub GraphQL API to avoid REST API pagination limits
...
class GitHubForksFetcher:
    def __init__(self, token=None):
        self.token = token
        self.headers = {
            "User-Agent": "Python-GraphQL-Forks-Fetcher",
    ...
    def save_data(self, data, owner, repo):
        ...
        filename = f"test/output/{owner}_{repo}_forks.json"
        
        with open(filename, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, separators=(",", ":"))
        
        print(f"Forks data saved to {filename}")
        return filename


def main():
    parser = argparse.ArgumentParser(description="Fetch GitHub repository forks data using GraphQL")
    parser.add_argument("url", help="GitHub repository URL")
    args = parser.parse_args()

    try:
        token = GitHubForksFetcher.get_token()
        if not token:
            sys.exit(1)

        fetcher = GitHubForksFetcher(token)
        owner, repo = fetcher.parse_url(args.url)

        print(f"Fetching forks data for {owner}/{repo} using GraphQL...")
